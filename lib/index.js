...
import { Card, Button, DatePicker, TimePicker, Radio, InputNumber, Rate, Tooltip, message } from 'antd';
...
class Index extends Component {
    ...
    getFormItemNode = v => {
        ...
        if (tpl === 'tabs') {
            ...
            formItemNode = <Tabs {...formItemNodeProps} />;
        }
        ...
        // TimePicker
        if (tpl === 'time-picker') {
            formItemNode = <TimePicker {...formItemNodeProps} />;
        }

        // TimeRangePicker
        if (tpl === 'time-range-picker') {
            formItemNode = <TimePicker.RangePicker {...formItemNodeProps} />;
        }
        ...
        // Slider
        if (tpl === 'rate') {
            formItemNode = <Rate {...formItemNodeProps} />;
        }
    ...
    renderColumns = () => {
        ...
        const columnsNode = columns
            ...
            .map((v, i) => {
                const { name, formListConfig, template } = v;
                ...
                if (formListConfig) {
                    ...
                    return (
                        <Form.List name={name} key={formItemProps.key} rules={rules}>
                            {(fields, action, { errors }) => {
                                const { add, remove, move } = action;
                                return (
                                    <Fragment>
                                                        ...
                                                        {fields.length < max && (
                                                            <Tooltip title="复制此行">
                                                                <CopyOutlined
                                                                    onClick={() => {
                                                                        const formRefNode = this.getFormRefNode();
                                                                        const value = formRefNode.getFieldValue([
                                                                            name,
                                                                            field.name
                                                                        ]);
                                                                        add(value, index + 1);
