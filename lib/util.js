...
export const mergeColumns = (columns = [], { disabled }) => {
    return cloneDeep(columns).map(v => {
        ...
        if (tpl === 'date-range-picker') {
            ...
            if (defaultValue) {
                column.defaultValue = defaultValue.map(v => {
                    return moment(v, template.format);
                });
            } else {
                column.defaultValue = [];
            }
        ...
        if (tpl === 'time-range-picker') {
            ...
            column.placeholder = undefined;
            console.log(111, column.defaultValue);
        }
...
export const validateColumns = (columns = []) => {
    columns.forEach(column => {
        ...
        if (tpl === 'input') {
            ...
            if (['select-search', 'select-input'].includes(inputType)) {
                ...
                if (isSomeFalsy(selectKey, inputKey)) {
                    throw new Error(
                        `[${componentName}] ${tpl} 必须传参数: "name" 形式为 "selectKey,inputKey"`
                    );
                }
            }
        }
        ...
        // 日期范围, 时间范围, 数字范围
        if (['date-range-picker', 'time-range-picker', 'range-number'].includes(tpl)) {
